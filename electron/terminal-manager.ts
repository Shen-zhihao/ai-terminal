...
export class TerminalManager {
  ...
  private shellIntegration = new ShellIntegration();
  ...
  create(
    ...
    if (!fs.existsSync(cwd)) {
      console.warn(
        `Working directory ${cwd} does not exist, falling back to HOME or /tmp`,
      );
      cwd =
    ...
    try {
      ...
      // Shell Integration (目前仅支持 Zsh)
      if (shell.endsWith("zsh")) {
        try {
          const zdotdir = this.shellIntegration.setupZsh();
          env.ZDOTDIR = zdotdir;
        } catch (e) {
          console.error("Failed to setup shell integration:", e);
        }
      }
      ...
      throw new Error(
        `Failed to spawn terminal (shell: ${shell}, cwd: ${cwd}): ${error.message}`,
      );
    }
  }
